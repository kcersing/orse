...
type ProductAttributeValueQuery struct {
	...
	// eager-loading edges.
	withKey   *ProductAttributeKeyQuery
	withItems *ProductSpecsItemQuery
...
func (pavq *ProductAttributeValueQuery) QueryKey() *ProductAttributeKeyQuery {
	query := &ProductAttributeKeyQuery{config: pavq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pavq.prepareQuery(ctx); err != nil {
			return nil, err
		}
		selector := pavq.sqlQuery(ctx)
		if err := selector.Err(); err != nil {
			return nil, err
		}
		step := sqlgraph.NewStep(
			sqlgraph.From(productattributevalue.Table, productattributevalue.FieldID, selector),
		...
		fromU = sqlgraph.SetNeighbors(pavq.driver.Dialect(), step)
		return fromU, nil
	}
	return query
}
...
func (pavq *ProductAttributeValueQuery) QueryItems() *ProductSpecsItemQuery {
	query := &ProductSpecsItemQuery{config: pavq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := pavq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
// Returns a *NotSingularError when exactly one ProductAttributeValue entity is not found.
...
// Returns a *NotSingularError when exactly one ProductAttributeValue ID is not found.
...
func (pavq *ProductAttributeValueQuery) Clone() *ProductAttributeValueQuery {
	...
	return &ProductAttributeValueQuery{
		...
		withKey:    pavq.withKey.Clone(),
		withItems:  pavq.withItems.Clone(),
...
func (pavq *ProductAttributeValueQuery) WithKey(opts ...func(*ProductAttributeKeyQuery)) *ProductAttributeValueQuery {
	query := &ProductAttributeKeyQuery{config: pavq.config}
...
func (pavq *ProductAttributeValueQuery) WithItems(opts ...func(*ProductSpecsItemQuery)) *ProductAttributeValueQuery {
	query := &ProductSpecsItemQuery{config: pavq.config}
	for _, opt := range opts {
		opt(query)
	}
	pavq.withItems = query
	return pavq
}
...
func (pavq *ProductAttributeValueQuery) sqlAll(ctx context.Context) ([]*ProductAttributeValue, error) {
	...
	if query := pavq.withKey; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*ProductAttributeValue)
		for i := range nodes {
			fk := nodes[i].KeyID
		...
		query.Where(productattributekey.IDIn(ids...))
		...
		for _, n := range neighbors {
			...
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "key_id" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Key = n
			}
	...
	if query := pavq.withItems; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		ids := make(map[int]*ProductAttributeValue, len(nodes))
		for _, node := range nodes {
			ids[node.ID] = node
			fks = append(fks, node.ID)
			node.Edges.Items = []*ProductSpecsItem{}
		}
		var (
			edgeids []int
			edges   = make(map[int][]*ProductAttributeValue)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(productattributevalue.ItemsPrimaryKey[1], fks...))
			},
		...
		if err := sqlgraph.QueryEdges(ctx, pavq.driver, _spec); err != nil {
			return nil, fmt.Errorf(`query edges "items": %w`, err)
		}
		query.Where(productspecsitem.IDIn(edgeids...))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		}
