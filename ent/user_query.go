...
func (uq *UserQuery) QueryDetail() *UserDetailQuery {
	query := &UserDetailQuery{config: uq.config}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withDetail: uq.withDetail.Clone(),
...
func (uq *UserQuery) WithDetail(opts ...func(*UserDetailQuery)) *UserQuery {
	query := &UserDetailQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDetail; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Detail = []*UserDetail{}
		}
		query.Where(predicate.UserDetail(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DetailColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			...
			node.Edges.Detail = append(node.Edges.Detail, n)
		}
	}
